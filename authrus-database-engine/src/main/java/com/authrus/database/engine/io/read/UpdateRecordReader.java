...
public class UpdateRecordReader implements ChangeRecordReader{
   
   private final String origin;
   private final String table;
   
   public UpdateRecordReader(String origin, String table) {
      this.origin = origin;
      this.table = table;
   } 
   ...
   public ChangeOperation read(DataRecordReader reader) throws IOException {
      ...
      if(count == 0) {
         throw new IllegalStateException("Update statement for '" + table + "' has no columns");
      } 
      ...
      return new UpdateOperation(origin, table, change);
   }
}
